...
class AIService {
  ...
  async generate(request: AIGenerationRequest): Promise<AIGenerationResponse> {
    ...
    // Check cache first
    const cached = aiCache.get(request.provider, request.prompt, request.model);
    if (cached) {
      recordMetric('ai.cache.hit', 1, { provider: request.provider });
      return cached;
    }
    ...
    try {
      ...
      if (result.success) {
        ...
        aiCache.set(request.provider, request.prompt, request.model, result);
  ...
  async generateText(params: {
    ...
    model?: string;
    maxTokens?: number;
  }): Promise<{ text: string; model: string; usage?: { totalTokens?: number; estimatedCost?: number } }> {
    ...
    return {
      text: typeof result.content === 'string' ? result.content : result.content?.join('\n') || '',
      model: params.model || 'unknown',
      usage: result.usage ? {
        totalTokens: result.usage.totalTokens,
        estimatedCost: result.usage.totalTokens ? (result.usage.totalTokens / 1000) * 0.002 : 0,
      } : undefined,
    };
  }
