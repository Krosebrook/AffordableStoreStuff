...
import { costTracker } from '../services/ai-cost-tracker';
...
router.get("/brand-voices/:id", async (req: Request, res: Response) => {
  try {
    const profile = await db.select().from(brandVoiceProfiles).where(eq(brandVoiceProfiles.id, String(req.params.id)));
    if (profile.length === 0) {
...
router.post("/brand-voices", requireAuth, async (req: Request, res: Response) => {
  try {
...
router.patch("/brand-voices/:id", requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    const [profile] = await db
      ...
      .where(eq(brandVoiceProfiles.id, String(req.params.id)))
      .returning();
...
router.delete("/brand-voices/:id", requireAuth, async (req: Request, res: Response) => {
  try {
    await db.delete(brandVoiceProfiles).where(eq(brandVoiceProfiles.id, String(req.params.id)));
    res.status(204).send();
  } catch (error) {
...
router.get("/product-concepts/:id", async (req: Request, res: Response) => {
  try {
    const concept = await db.select().from(productConcepts).where(eq(productConcepts.id, String(req.params.id)));
    if (concept.length === 0) {
...
router.post("/product-concepts/generate", requireAuth, async (req: Request, res: Response) => {
  try {
...
router.patch("/product-concepts/:id", requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    const [concept] = await db
      ...
      .set({ ...parsed.data, updatedAt: new Date() })
      .where(eq(productConcepts.id, String(req.params.id)))
      .returning();
...
router.delete("/product-concepts/:id", requireAuth, async (req: Request, res: Response) => {
  try {
    await db.delete(productConcepts).where(eq(productConcepts.id, String(req.params.id)));
    res.status(204).send();
  } catch (error) {
...
router.get("/campaigns/:id", async (req: Request, res: Response) => {
  try {
    const campaign = await db.select().from(marketingCampaigns).where(eq(marketingCampaigns.id, String(req.params.id)));
...
router.post("/campaigns/:id/generate-assets", requireAuth, async (req: Request, res: Response) => {
  try {
    const campaign = await db.select().from(marketingCampaigns).where(eq(marketingCampaigns.id, String(req.params.id)));
    if (campaign.length === 0) {
      return res.status(404).json({ error: "Campaign not found" });
    ...
    const [updated] = await db
      ...
      .where(eq(marketingCampaigns.id, String(req.params.id)))
      .returning();
...
router.post("/content-library", requireAuth, async (req: Request, res: Response) => {
  try {
...
router.patch("/content-library/:id/favorite", requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    const [item] = await db
      ...
      .where(eq(aiContentLibrary.id, String(req.params.id)))
      .returning();
...
// Get session cost
router.get("/cost/session/:id", (req: Request, res: Response) => {
  res.json(costTracker.getSessionCost(String(req.params.id)));
});

// Get aggregate costs
router.get("/cost/aggregate", (req: Request, res: Response) => {
  const since = req.query.since ? parseInt(req.query.since as string) : undefined;
  res.json(costTracker.getAggregateCost(since));
});

// Batch generation (up to 10 requests)
router.post("/batch/generate", async (req: Request, res: Response) => {
  try {
    ...
    const response = results.map((result, index) => {
      ...
      return { index, success: false, error: result.reason?.message || "Failed" };
    });

    res.json({ results: response, total: requests.length });
  } catch (error) {
